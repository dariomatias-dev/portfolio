...
export const TechStackListItem = ({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "w-full text-left flex items-center gap-4 p-3 rounded-xl border transition-all duration-300 group relative overflow-hidden",
        isSelected
    ...
    >
      <div
        className={cn(
          "absolute left-0 top-1/2 -translate-y-1/2 w-1 bg-blue-600 rounded-r-full origin-center transition-all duration-300 ease-out",
      ...
      <div className="flex-1 z-10 min-w-0">
        <div className="flex items-center justify-between">
          <h4
            className={cn(
              "font-bold text-sm truncate pr-2 transition-colors duration-300",
              isSelected
                ? "text-slate-900"
          ...
          <ArrowRight
            size={14}
            className={cn(
              "shrink-0 transition-all duration-300 ease-out group-hover:translate-x-1"
            )}
          />
        </div>
      </div>
